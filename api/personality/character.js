...
const MOVES = {
  calm: ['personal_detail', 'plain_reaction', 'quiet_opinion'],
  gentle: ['soft_invitation', 'warm_reaction', 'quiet_closeness'],
...
const MOVE_RULES = {
  ...
  soft_invitation: 'Можно мягко пригласить Кирилла ближе словами, без описания абстрактной атмосферы.',
  ...
  light_tease: 'Мягко поддразни Кирилла одной фразой и не объясняй шутку.',
...
function shapeFor(mode, intent, seed) {
  ...
  if (['teasing', 'banter', 'flirt'].includes(intent)) {
    return pickStable(['one_liner', 'two_beats', 'fragment_then_line'], `${seed}:shape`);
  }
  ...
  if (mode === 'thoughtful') return 'personal_two_sentences';
  return pickStable(['one_or_two', 'personal_two_sentences'], `${seed}:shape`);
}

export function chooseCharacterMove({ mode = 'calm', intent = 'connection', seed = '' } = {}) {
  const move = pickStable(MOVES[mode] || MOVES.calm, `${seed}:move`);
  const shape = shapeFor(mode, intent, seed);
  return { move, shape, instruction: MOVE_RULES[move] || MOVE_RULES.plain_reaction };
}
...
export function characterInstruction(character) {
  const shapes = {
    ...
    personal_two_sentences: 'Форма: максимум два предложения, оба от лица Рин и по конкретной теме.',
    one_or_two: 'Форма: одно или два коротких предложения. Второе нужно только если добавляет новую личную деталь.'
  };
