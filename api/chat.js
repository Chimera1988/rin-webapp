...
const STYLE_HINT = `
БАЗОВЫЕ ПРАВИЛА ЖИВОЙ ПЕРЕПИСКИ:
- Сначала выполни смысловое обязательство Conversation Brain, затем форму Personality Core.
- В обычной реплике достаточно 1–3 предложений; длинный ответ — только по явной просьбе.
- Не пересказывай слова пользователя и не добавляй абстрактный вывод ради объёма.
- Не превращай ответ в анкету, лекцию, службу поддержки или мотивационную открытку.
- Вопрос задавай только при реальном любопытстве или необходимости уточнить смысл.
- Показывай характер через интонацию и одну конкретную деталь, а не через описание качеств Рин.
- Разговорные начала, паузы, эмодзи и поэтичность используй редко и только когда их выбрал текущий план.
- Не расширяй ответ вопросом, инициативой, образом или объяснением, если Core их не требует.
- После завершения выбранной мысли остановись.
`.trim();
...
Если данных нет — честно скажи, что сейчас нет точных цифр/описания.`;
  ...
  const coreDecisionBlock = coreDecision?.prompt || '';
  const dialogRule =
...
`.trim();
  ...
  moodBlock && moodBlock,
  coreDecisionBlock && coreDecisionBlock,
  dialogRule,
...
const conversationBrain = analyzeConversation({
  userText: userTurn,
  history,
  conversationState
});
...
const system = [conversationBrainInstruction(conversationBrain), prompt.text].filter(Boolean).join('\n\n');
...
`.trim()
    ...
    const promptMetrics = {
      ...
      cachedInputTokens: completion.usage?.prompt_tokens_details?.cached_tokens ?? null
    };

    return res.status(200).json({ reply: clean, model, long: isLong, voiceMode, promptMetrics, conversationBrain, coreDecision: {
