...
function deriveSignals(userText, replyText, context = {}) {
  ...
  const affection = flirt || tenderness || hug || kiss;
  ...
  if (praise || thanks || care || tenderness || tease || reflection || memory || tired) intensity = Math.max(intensity, 2);
  ...
  return { scene, intensity, farewell, greeting, greeting_only, kiss, hug, flirt, tease, praise, thanks, apology, regret, agreement, neutral_ack, tired, sad, angry, frustrated, disappointment, surprise, question, confusion, memory, dreamy, reflection, shy, jealousy, care, tenderness, invitation, hope, pride, interest, affection, celebration, strong_joy, light_joy, informational };
}
...
function isMeaningfulStickerMoment(sig) {
  ...
  return sig.kiss || sig.hug || sig.surprise || sig.celebration || sig.apology || sig.sad || sig.angry || sig.flirt || sig.tease || sig.praise || sig.thanks || sig.tired || sig.memory || sig.reflection || sig.question || sig.agreement;
}

export function decideSticker(config, { userText = '', replyText = '', mood = {}, mode = 'smart', context = {} } = {}) {
  ...
  if (mode !== 'always' && ratio > Number(config.defaults?.maxRatio ?? 0.26)) return { action: 'none', reason: 'ratio_gate', signals: sig };

  const sceneChance = Number(config.defaults?.sceneChance?.[sig.scene] ?? 0.16);
  if (mode !== 'always' && Math.random() > sceneChance) return { action: 'none', reason: 'scene_probability_gate', signals: sig };
  ...
  for (const sticker of config.stickers || []) {
    ...
    if (recent.has(sticker.src)) candidate.score -= 0.8;
  ...
  if (scored[1] && Math.abs(best.score - scored[1].score) < 0.1 && best.sticker.family !== scored[1].sticker.family) {
