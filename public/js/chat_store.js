...
export function reconcilePendingDeliveryHistory(history, committedRequestId = '') {
  const committed = clean(committedRequestId, 120);
  return normalizeStoredHistory(history)
    .filter(message => !(message.role === 'assistant' && message.status === 'pending' && message.deliveryId && (!committed || message.requestId !== committed)))
    .map(message => (
      committed && message.role === 'user' && message.requestId === committed && message.status !== 'complete'
        ? normalizeStoredMessage({ ...message, status: 'complete', errorCode: null })
        : message
    ));
}
...
export function assignMessageBatch(history, messageIds = [], options = {}) {
  const ids = new Set((Array.isArray(messageIds) ? messageIds : []).map(String));
  const hasRequestId = Object.prototype.hasOwnProperty.call(options, 'requestId');
  const hasTurnId = Object.prototype.hasOwnProperty.call(options, 'turnId');
  const status = options.status || 'pending';
  const changed = [];
  for (const message of history) {
    if (!ids.has(String(message?.id || '')) || message?.role !== 'user') continue;
    const next = normalizeStoredMessage({
      ...message,
      requestId: hasRequestId ? options.requestId : message.requestId,
      turnId: hasTurnId ? options.turnId : message.turnId,
      status,
      errorCode: null
    });
    if (!next) continue;
