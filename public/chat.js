...
const RIN_BUILD_VERSION = '2026-07-29-identity-mood-stickers-v2';
...
async function maybeSticker(userText, replyText, poolOverride = null){
  ...
  try{
    ...
    if (stickersLib && STICKERS_CFG){
      ...
      const diag = decision?.diagnostic;
      if (diag) {
        const top = Array.isArray(diag.topCandidates)
          ? diag.topCandidates
              .slice(0, 3)
              .map(x => `${x.src}:${Number(x.score || 0).toFixed(3)}`)
              .join(', ')
          : 'none';
        dbg(
          `stickers v3 no-decision: reason=${diag.reason}; ` +
          `intent=${diag.signals?.intent}; sentiment=${diag.signals?.sentiment}; ` +
          `gap=${diag.history?.messagesSinceSticker}; ratio=${Number(diag.history?.ratio || 0).toFixed(2)}; ` +
          `threshold=${diag.threshold}; top=[${top}]`
        );
      } else {
        dbg('stickers v3 no-decision: no diagnostic');
      }
}
