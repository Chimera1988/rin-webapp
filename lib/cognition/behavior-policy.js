...
import { advancePersistentIntent } from './persistent-intent.js';
...
function asksForEvidenceOfRinClaim(text = '', brain = null) {
  const t = cleanText(text, 1200).toLowerCase();
  if (brain?.relation?.type !== 'follow_up_on_rin_statement' && brain?.relation?.type !== 'new_or_followup_question') return false;
  return /(?:откуда|почему\s+ты\s+(?:так\s+)?(?:дума|реш)|когда\s+я|где\s+я|в\s+каких\s+моментах|какие\s+примеры|на\s+ч[её]м.*вывод|основан.*вывод|с\s+чего\s+ты)/iu.test(t);
}
function chooseResponseAct({ brain, state, relation, userText, inputReplyTarget, emotionalState, affectiveSignal, persistentIntent }) {
  ...
  if (asksForEvidenceOfRinClaim(text, brain)) return 'explain_belief_basis';
  ...
  // Ambiguity is allowed to interrupt the scene only when it is genuinely critical.
  // A stored hook, a committed intent, or a confident continuity binding is better
  // evidence than the lexical ambiguity heuristic.
  const criticalAmbiguity = Boolean(
    brain?.ambiguity?.shouldClarify
    && literal === 'question'
    && persistentIntent?.status !== 'active'
    && !state?.openHook
    && Number(state?.confidence || 0) < 0.58
    && ['new_or_followup_question', 'new_topic'].includes(brain?.relation?.type || 'new_or_followup_question')
  );
  if (criticalAmbiguity) return 'clarify_critical_ambiguity';
  ...
  // PersistentIntent owns multi-turn goal continuity. Scene heuristics may colour the
  // act but cannot replace a committed goal with a generic playful action.
  if (persistentIntent?.status === 'active' && Number(persistentIntent?.commitment) >= 55) return 'advance_persistent_intent';
...
function actionForAct(responseAct) {
  ...
  if (['answer_directly', 'answer_selected_message', 'explain_previous_nonverbal', 'acknowledge_correction', 'explain_belief_basis'].includes(responseAct)) return 'answer';
  ...
  if (['take_lead', 'reclaim_scene', 'tease_and_advance', 'advance_play', 'carry_playful_tension', 'advance_persistent_intent'].includes(responseAct)) return 'continue_scene';
  ...
  if (['proactive_greeting','proactive_callback','proactive_personal_share'].includes(responseAct)) return 'initiate';
  return 'react';
}

function chooseInitiative({ responseAct, brain, cognition, relation, userText, history, state, emotionalState, persistentIntent }) {
  const scene = state?.scene || brain?.activeScene?.type || 'everyday';
  if (responseAct === 'advance_persistent_intent' && persistentIntent?.status === 'active') return { mode: 'persistent_goal', strength: Math.max(60, Number(persistentIntent.commitment) || 60), reason: 'продолжение уже committed собственного намерения Рин' };
  ...
  if (['name_emotion_if_asked', 'clarify_self', 'reassure_with_boundary', 'explain_previous_nonverbal', 'explain_belief_basis'].includes(responseAct)) {
...
function questionPolicy({ brain, state, responseAct, initiative, history }) {
  ...
  if (responseAct === 'advance_persistent_intent') return { budget: 0, reason: 'активная собственная цель продвигается действием, а не передачей управления пользователю' };
...
function expressionFor({ responseAct, emotionalState }) {
  ...
  if (['carry_playful_tension', 'take_lead', 'tease_and_advance', 'advance_play', 'playful_stance', 'advance_persistent_intent'].includes(responseAct)) return 'behavior_first';
...
function toneFor({ scene, responseAct, relation }) {
  ...
  if (['take_lead', 'reclaim_scene', 'tease_and_advance', 'advance_play', 'playful_stance', 'carry_playful_tension', 'advance_persistent_intent'].includes(responseAct)) return 'warm_bold_playful';
...
function directnessFor({ scene, responseAct, relation, relationType }) {
  ...
  if (['take_lead', 'reclaim_scene', 'tease_and_advance', 'advance_play', 'playful_stance', 'carry_playful_tension', 'advance_persistent_intent', 'contained_jealousy'].includes(responseAct)) return 'confident_playful';
...
export function deriveBehaviorPolicy({ cognition = null, brain = null, coreDecision = null, memory = null, userText = '', history = [], trigger = null } = {}) {
  ...
  const persistentIntent = advancePersistentIntent({ memory, characterIntent, dialogueState: state, brain, userText });
  const relationshipIntent = deriveRelationshipIntent({ memory, history, dialogueState: state, affectiveTurn });
  const director = directConversation({ userText, brain, dialogueState: state, history, explicitReplyTarget: inputReplyTarget, characterIntent, persistentIntent, relationshipIntent, emotionalState });
  const proactiveAct = trigger?.type
    ? (persistentIntent?.status === 'active' ? 'advance_persistent_intent'
      : trigger.type === 'greeting' ? 'proactive_greeting'
        : cognition?.openLoops?.callback ? 'proactive_callback' : 'proactive_personal_share')
    : null;
  const responseAct = trigger?.type
    ? proactiveAct
    : director.delivery === 'silence'
      ? 'intentional_silence'
      : chooseResponseAct({ brain, state, relation, userText, inputReplyTarget, emotionalState, affectiveSignal: affectiveTurn?.signal, persistentIntent });
  const initiative = trigger?.type
    ? { mode: 'proactive', strength: 82, reason: 'самостоятельный ход Рин инициирован единым TurnDecision pipeline' }
    : director.delivery === 'silence'
      ? { mode: 'none', strength: 0, reason: 'выбрано осознанное молчание' }
      : chooseInitiative({ responseAct, brain, cognition, relation, userText, history, state, emotionalState, persistentIntent });
  const question = trigger?.type
    ? { budget: responseAct === 'proactive_callback' ? 1 : 0, reason: responseAct === 'proactive_callback' ? 'один конкретный callback-вопрос допустим по незакрытой линии' : 'proactive ход должен быть самостоятельным, не generic check-in вопросом' }
    : questionPolicy({ brain, state, responseAct, initiative, history });
  const scene = state?.scene || brain?.activeScene?.type || 'everyday';
  const action = actionForAct(responseAct);
  const emotionalExpression = expressionFor({ responseAct, emotionalState });
  const topicHold = persistentIntent?.status === 'active' ? 'hold_persistent_intent' : state?.topicDrift ? 'reclaim' : state?.openHook ? 'hold_open_hook' : 'hold_scene';
  const distance = ['contained_jealousy', 'hold_emotional_boundary'].includes(responseAct) ? 'slightly_withdrawn'
    : ['reciprocate_closeness', 'personal_closeness', 'soften_after_repair'].includes(responseAct) ? 'closer'
      : 'stable';
  const playfulness = ['take_lead', 'reclaim_scene', 'tease_and_advance', 'advance_play', 'playful_stance', 'warm_playful_reply', 'carry_playful_tension', 'advance_persistent_intent'].includes(responseAct)
  ...
  return {
    version: 'rin-behavior-policy-v4-single-owner',
    action,
    responseAct,
    ...
    reasons: [
      ...
      `topicHold:${topicHold}`,
      `distance:${distance}`,
      `persistentIntent:${persistentIntent?.status || 'none'}:${persistentIntent?.goal || ''}`
    ]
  };
}
...
export function behaviorPolicyInstruction(policy = {}) {
  return [
    'BEHAVIOR POLICY v3 — ДЕЙСТВИЕ + SCENE-BOUND PERSISTENT INTENT',
    ...
    policy.persistentIntent?.status === 'active' ? `Активная цель Рин: ${policy.persistentIntent.goal}; следующий ход: ${policy.persistentIntent.nextMove}; binding: ${policy.persistentIntent.sceneBinding?.key || 'fallback'}; progress ${Math.round((Number(policy.persistentIntent.progress) || 0) * 100)}%.` : 'Активной долгоживущей цели Рин нет.',
