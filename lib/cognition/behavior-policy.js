...
export function deriveBehaviorPolicy({ cognition = null, brain = null, coreDecision = null, memory = null, userText = '', history = [] } = {}) {
  ...
  return {
    version: 'rin-behavior-policy-v2-persistent-intent',
    action,
    responseAct,
...
export function behaviorPolicyInstruction(policy = {}) {
  return [
    'BEHAVIOR POLICY v2 — ДЕЙСТВИЕ + PERSISTENT INTENT',
    ...
    policy.persistentIntent?.status === 'active' ? `Активная цель Рин: ${policy.persistentIntent.goal}; следующий ход: ${policy.persistentIntent.nextMove}; progress ${Math.round((Number(policy.persistentIntent.progress) || 0) * 100)}%.` : 'Активной долгоживущей цели Рин нет.',
