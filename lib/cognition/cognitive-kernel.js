...
import { STICKER_INTENT_VALUES, stickerIntentGuideText } from './sticker-catalog.js';
...
function decisionStateSnapshot(state = {}, visualReplyCandidates = []) {
  return {
    ...
    openLoops: Array.isArray(state?.openLoops) ? state.openLoops.slice(0, 5) : [],
    replyTarget: state?.replyTarget,
    visualReplyCandidates: Array.isArray(visualReplyCandidates) ? visualReplyCandidates.slice(0, 4) : [],
    environment: state?.environment,
    stickerState: state?.stickerState,
    reciprocity: state?.reciprocity,
    lore: state?.lore ? {
      canon: Array.isArray(state.lore.canon) ? state.lore.canon.slice(0, 3) : [],
      memories: Array.isArray(state.lore.memories) ? state.lore.memories.slice(0, 2) : [],
      backstory: Array.isArray(state.lore.backstory) ? state.lore.backstory.slice(0, 2) : []
    } : null,
    recentHistory: Array.isArray(state?.recentHistory) ? state.recentHistory.slice(-6) : [],
    userEvents: Array.isArray(state?.userEvents) ? state.userEvents.slice(-4) : []
  };
}
...
export function buildKernelPrompt({ profile = null, state = null, client = null, trigger = null } = {}) {
  ...
  const stickerGuide = allowStickers ? stickerIntentGuideText() : '';
  ...
  const system = [
    ...
    `Текущий state:\n${JSON.stringify(decisionStateSnapshot(state, visualReplyCandidates)).slice(0, 12000)}`,
    `StickerState: ${JSON.stringify(stickerState || { available: false, reason: 'missing_sticker_state' })}. Частота/cooldown уже рассчитаны до тебя и являются hard availability, а не пожеланием. Если available=false, schema физически не позволяет sticker. Если available=true, стикер всё равно НЕ является украшением по умолчанию: выбирай его только когда он добавляет самостоятельный невербальный beat. ${allowStickers ? `Допустимые semantic stickerIntent: ${STICKER_INTENT_VALUES.join(', ')}.` : 'На этом ходе stickerIntent недоступен.'} Для text segment stickerIntent=null.`,
    stickerGuide ? `ТОЧНАЯ СЕМАНТИКА СТИКЕРОВ — SSOT manifest. stickerIntent является конкретным смысловым жестом и одновременно точным asset id; family не является intent. Выбирай наиболее точный вариант по текущему контексту, а не общий тип жеста:\n${stickerGuide}` : '',
    ...
    '- Sticker segment использует только один ТОЧНЫЙ stickerIntent из разрешённой semantic map выше; не придумывай новые названия и не подменяй конкретный смысл широким family вроде kiss/tender/flirt/hug/playful.',
    '- stickerState.recentAssetIds — только контекст недавнего использования. Если несколько ТОЧНЫХ intent одинаково уместны, можно предпочесть неиспользованный недавно; никогда не меняй смысл жеста только ради визуальной ротации.',
