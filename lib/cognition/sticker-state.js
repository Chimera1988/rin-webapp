import { stickerCatalogDefaults, stickerCatalogItems } from './sticker-catalog.js';
...
function consecutiveStickerTurns(turns = []) {
  let count = 0;
  for (let index = turns.length - 1; index >= 0; index -= 1) {
    if (!turns[index]?.hasSticker) break;
    count += 1;
  }
  return count;
}

function recentFamilySequence(assetIds = [], max = 12) {
  const byId = new Map(stickerCatalogItems().map(item => [clean(item?.id, 80).toLowerCase(), clean(item?.family, 80)]));
  return (Array.isArray(assetIds) ? assetIds : [])
    .map(id => byId.get(clean(id, 80).toLowerCase()) || null)
    .filter(Boolean)
    .slice(0, max);
}
...
export async function buildStickerState({ history = [], preference = null, scene = 'everyday', userText = '' } = {}) {
  ...
  // Two levels are intentionally exposed:
  // - hardAvailable: the feature is technically/policy permitted;
  // - available: Rin Mind may actually choose a sticker on this turn.
  // In smart mode frequency and cooldown are real behavioral gates. Explicit
  // kiss/hug gestures may exceed the rolling target because mirroring such a
  // gesture is semantically stronger than a decorative sticker budget.
  ...
  } else if (mode === 'smart' && remainingStickerTurns <= 0 && !explicitGesture) {
    available = false;
  ...
  } else if (explicitGesture && mode === 'smart' && remainingStickerTurns <= 0) {
    reason = 'explicit_gesture_override';
  }
  ...
  const recentFamilies = recentFamilySequence(recentAssetIds, recentAssetWindow);
  const stickerStreak = consecutiveStickerTurns(turns);
  return {
    ...
    recentEmotions,
    recentFamilies,
    consecutiveStickerTurns: stickerStreak
  };
}
