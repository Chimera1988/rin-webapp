...
function visualReplyCandidatesFromEvents(userEvents = []) {
  const events = Array.isArray(userEvents) ? userEvents : [];
  if (events.length < 2) return [];
  // A visual quote is useful only to anchor an earlier event when newer user
  // messages exist. The latest event is already visually adjacent and must not
  // be auto-quoted as part of an ordinary reply.
  return events.slice(0, -1).map(item => ({
    eventId: clean(item.id, 120),
    excerpt: clean(item.content, 360)
  })).filter(item => item.eventId && item.excerpt).slice(0, 4);
}
...
function conversationTurns(history = []) {
  const rows = (Array.isArray(history) ? history : [])
    ...
    .filter(item => !['sticker', 'silence'].includes(item?.kind || 'text'))
    .filter(item => ['sent', 'complete'].includes(item?.status || 'complete'));
  const turns = [];
  for (let index = 0; index < rows.length; index += 1) {
    const item = rows[index];
    const identity = clean(item?.turnId, 120) || clean(item?.requestId, 120) || clean(item?.id, 120) || `row-${index}`;
    const key = `${item.role}:${identity}`;
    const content = clean(item?.content, 1400);
    const previous = turns.at(-1);
    if (previous?.key === key) {
      if (content) previous.contents.push(content);
      continue;
    }
    turns.push({ key, role: item.role, contents: content ? [content] : [] });
  }
...
const PERSONAL_QUESTION = /(?:как\s+(?:ты|тебе|твой|твоя|твои|тво[её]|себя|дела|настроение|самочувствие|день|вечер|утро|ночь)|как\s+(?:прош[её]л|ид[её]т)\s+(?:твой|у\s+тебя)|что\s+ты\s+(?:делаешь|чувствуешь)|чем\s+(?:ты\s+)?занимаешься|у\s+тебя\s+(?:как|что|вс[её]\s+ли)|ты\s+(?:устала|устал|занята|занят|в\s+порядке))/iu;

function reciprocitySnapshot(history = [], brain = null, conversationState = 'ongoing', currentUserText = '') {
  // Count conversational turns, not rendered bubbles. Multi-message assistant
  // deliveries and rapid user batches share a request/turn id and must not
  // distort the social-reciprocity window.
  const recent = conversationTurns(history).slice(-12);
  const userTurns = recent.filter(item => item.role === 'user');
  const rinTurns = recent.filter(item => item.role === 'assistant');
  const hasQuestion = item => /\?/u.test(String(item?.content || ''));
  const userQuestionTurns = userTurns.filter(hasQuestion).length;
  const rinQuestionTurns = rinTurns.filter(hasQuestion).length;
  const currentUser = clean(currentUserText, 4000) ? { role: 'user', content: clean(currentUserText, 4000) } : null;
  const currentUserQuestion = Boolean(currentUser && hasQuestion(currentUser));
  const currentUserPersonalQuestion = Boolean(currentUserQuestion && PERSONAL_QUESTION.test(String(currentUser?.content || '')));

  let lastRinQuestionDistance = null;
  for (let index = recent.length - 1, distance = 0; index >= 0; index -= 1, distance += 1) {
    const item = recent[index];
    if (item.role === 'assistant' && hasQuestion(item)) {
      lastRinQuestionDistance = distance;
      break;
    }
  }
  const rinAskedRecently = lastRinQuestionDistance != null && lastRinQuestionDistance <= 4;
  const oneSidedQuestionPattern = userTurns.length >= 2 && rinTurns.length >= 1 && userQuestionTurns >= 2 && rinQuestionTurns === 0;
  const scene = clean(brain?.activeScene?.type, 80) || 'everyday';
  const casualScene = !['practical_task', 'conflict_repair'].includes(scene);
  const directPersonalOpportunity = Boolean(currentUserPersonalQuestion && !rinAskedRecently);
  const accumulatedOpportunity = Boolean(oneSidedQuestionPattern && currentUserQuestion && !rinAskedRecently);
  const reciprocalQuestionExpected = Boolean(
    casualScene
    && conversationState !== 'ending'
    && (directPersonalOpportunity || accumulatedOpportunity)
  );
  const reason = directPersonalOpportunity
    ? 'direct_personal_interest'
    : accumulatedOpportunity ? 'one_sided_question_pattern' : null;
  return {
    windowTurns: recent.length,
    userTurns: userTurns.length,
    rinTurns: rinTurns.length,
    userQuestionTurns,
    rinQuestionTurns,
    currentUserQuestion,
    currentUserPersonalQuestion,
    rinAskedRecently,
    lastRinQuestionDistance,
    oneSidedQuestionPattern,
    reciprocalQuestionExpected,
    reciprocalQuestionReason: reciprocalQuestionExpected ? reason : null,
    questionAnchor: reciprocalQuestionExpected ? clean(currentUser?.content, 280) : null
  };
}
...
export function buildKernelState({
  ...
  const recentIntents = (Array.isArray(memory?.conversationState?.recentIntents) ? memory.conversationState.recentIntents : [])
    .map(normalizeRinIntent)
    .filter(intent => intent && ['completed', 'cancelled'].includes(intent.status))
    .slice(-6);
  const userEvents = userEventsFromHistory(history, requestId);
  const visualReplyCandidates = visualReplyCandidatesFromEvents(userEvents);
  return {
    ...
    reciprocity: reciprocitySnapshot(history, brain, conversationState, userText),
    lore: {
...
export function compactKernelState(state = {}) {
  return {
    ...
    visualReplyCandidates: state.visualReplyCandidates,
