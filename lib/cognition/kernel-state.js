...
function reciprocitySnapshot(history = [], brain = null, conversationState = 'ongoing') {
  ...
  const currentUser = [...recent].reverse().find(item => item.role === 'user') || null;
  const currentUserQuestion = Boolean(currentUser && hasQuestion(currentUser));
  const oneSidedQuestionPattern = userTurns.length >= 2 && rinTurns.length >= 2 && userQuestionTurns >= 2 && rinQuestionTurns === 0;
...
export function buildKernelState({
  ...
  return {
    ...
    reciprocity: reciprocitySnapshot(history, brain, conversationState),
    lore: {
