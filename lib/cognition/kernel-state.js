...
function reciprocitySnapshot(history = []) {
  ...
  return {
    ...
    userQuestionTurns,
    rinQuestionTurns,
    // This is an observation, never a quota. Kernel still decides whether a
    // concrete detail actually creates genuine curiosity on the current turn.
    oneSidedQuestionPattern: userTurns.length >= 2 && rinTurns.length >= 2 && userQuestionTurns >= 2 && rinQuestionTurns === 0
  };
}
...
export function buildKernelState({
  ...
  return {
    ...
    reciprocity: reciprocitySnapshot(history),
    lore: {
