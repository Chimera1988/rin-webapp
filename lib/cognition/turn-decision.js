...
export function applyIntentTransition(currentInput = null, decisionInput = null, { revision = 0, scene = 'everyday' } = {}) {
  ...
  if (operation === 'activate') {
    ...
    const maxTurns = Math.max(6, Math.min(10, 6 + Math.floor(Math.max(0, commitment - 60) / 10)));
  ...
  if (operation === 'preserve') {
    return normalizeRinIntent({
      ...
      progressState: current.status === 'suspended' ? 'suspended' : 'preserved',
...
export function buildDecisionStateTransition({ kernelState = null, affectiveTurn = null, decision = null, now = Date.now() } = {}) {
  ...
  const dialogueState = state.dialogueState && typeof state.dialogueState === 'object'
    ? { ...state.dialogueState, recentActs: [...priorActs, ...(currentAct ? [currentAct] : [])].slice(-8) }
    : (currentAct ? { recentActs: [currentAct] } : null);
