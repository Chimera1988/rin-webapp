...
import { persistentIntentInstruction } from './persistent-intent.js';
...
function goalOf(brain = null, inputReplyTarget = null, responseAct = 'direct_response') {
  ...
  if (responseAct === 'advance_persistent_intent') return 'продвинуть уже начатую собственную цель Рин конкретным следующим ходом и не передавать управление обратно пользователю';
  if (responseAct === 'proactive_greeting') return 'самой начать новый контакт короткой личной репликой без generic интервью';
  if (responseAct === 'proactive_callback') return 'самой вернуться к конкретной незавершённой детали, потому что она действительно релевантна';
  if (responseAct === 'proactive_personal_share') return 'самой внести одну конкретную текущую деталь/мысль Рин без требования ответа';
...
function forbiddenPatterns({ responseAct, state, scene, emotionalState = null, rinIntent = null }) {
  const out = [
  ...
  if (['take_lead', 'reclaim_scene', 'tease_and_advance', 'advance_play', 'carry_playful_tension', 'advance_persistent_intent'].includes(responseAct)) {
  ...
  if (rinIntent?.status === 'active') {
    out.push('Не сбрасывать незавершённую собственную цель Рин в «о чём хочешь поговорить?», «давай просто поболтаем», «выбирай тему» или другое перекладывание инициативы.');
    out.push('Не объявлять активную цель завершённой раньше completion condition и не обещать продолжить её когда-нибудь позже вместо текущего шага.');
  }
  return uniqueStrings(out, 10, 500);
}

export function planResponse({ cognition = null, brain = null, coreDecision = null, memory = null, userText = '', history = [], isLong = false, trigger = null } = {}) {
  ...
  const behavior = deriveBehaviorPolicy({ cognition, brain, coreDecision, memory, userText, history, trigger });
  ...
  const rinIntent = behavior.persistentIntent || null;
  ...
  const mustAddress = uniqueStrings([
    ...
    rinIntent?.status === 'active' ? `Активная собственная цель Рин: ${rinIntent.goal}. Продвинуть её следующим ходом: ${rinIntent.nextMove}.${rinIntent.sceneBinding?.key ? ` Конкретная привязка: ${rinIntent.sceneBinding.key}${rinIntent.sceneBinding.anchor ? ` — «${rinIntent.sceneBinding.anchor}»` : ''}.` : ''}` : ''
  ], 10, 500);
  ...
  return normalizeResponsePlan({
    ...
    threadPolicy: rinIntent?.status === 'active'
      ? `Долгоживущая цель Рин «${rinIntent.goal}» сохраняется между релевантными ходами до completion/abandonment; прямой вопрос пользователя можно сначала закрыть, не стирая эту цель.`
      : state?.openHook
    ...
    mustNot: forbiddenPatterns({ responseAct, state, scene, emotionalState, rinIntent }),
    inputReplyTarget,
    replyTarget,
    delivery: trigger?.type ? (coreDecision?.nonverbalAction?.delivery || 'text') : director?.delivery === 'silence' ? 'silence' : (coreDecision?.nonverbalAction?.delivery || 'text'),
    ...
    reasons: [
      ...
      `rinIntent:${rinIntent?.status || 'none'}:${rinIntent?.goal || ''}`
    ]
  });
}
...
function actInstruction(act = 'direct_response') {
  const map = {
    ...
    advance_persistent_intent: 'Продвинь активную цель Рин конкретным шагом по её sceneBinding/target. Не подменяй конкретный предмет общей игривостью. Не сворачивай её в «о чём хочешь поговорить?», не комментируй сам разговор и не обещай когда-нибудь продолжить — действие должно произойти сейчас.',
    proactive_greeting: 'Рин сама начинает контакт. Одна короткая конкретная реплика от себя; не используй «как дела?» как обязательный шаблон и не создавай биографическую деталь без Inner Life/canon.',
    proactive_callback: 'Рин сама возвращается к действительно незакрытой конкретной детали. Допустим максимум один точный вопрос именно о ней; не изображай память ради эффекта.',
    proactive_personal_share: 'Рин сама вносит одну текущую мысль или сохранённую Inner Life деталь. Не задавай generic вопрос и не выдумывай новое занятие.',
...
export function responsePlanInstruction(plan = {}) {
  ...
  const initiative = {
    ...
    persistent_goal: 'Продвинь активную локальную цель Рин конкретным действием; не отдавай выбор темы обратно пользователю.',
...
${persistentIntentInstruction(plan.rinIntent)}
