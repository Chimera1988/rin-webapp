...
export function planResponse({ cognition = null, brain = null, coreDecision = null, memory = null, userText = '', history = [], isLong = false } = {}) {
  ...
  const mustAddress = uniqueStrings([
    ...
    rinIntent?.status === 'active' ? `Активная собственная цель Рин: ${rinIntent.goal}. Продвинуть её следующим ходом: ${rinIntent.nextMove}.` : ''
  ], 10, 500);
...
function actInstruction(act = 'direct_response') {
  const map = {
    ...
    advance_persistent_intent: 'Продвинь активную цель Рин конкретным шагом. Не сворачивай её в «о чём хочешь поговорить?», не комментируй сам разговор и не обещай когда-нибудь продолжить — действие должно произойти сейчас.',
