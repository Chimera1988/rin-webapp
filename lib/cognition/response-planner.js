...
export function planResponse({ cognition = null, brain = null, coreDecision = null, memory = null, userText = '', history = [], isLong = false } = {}) {
  ...
  const characterIntent = deriveCharacterIntent({ userText, dialogueState: state, brain, memory });
  const relationshipIntent = deriveRelationshipIntent({ memory, history, dialogueState: state });
  ...
  const responseAct = director.delivery === 'silence' ? 'intentional_silence' : responseActOf({ brain, state, relation, userText, inputReplyTarget });
