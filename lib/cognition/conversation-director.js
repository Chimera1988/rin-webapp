...
function silenceEligibility({ userText, brain, dialogueState, explicitReplyTarget, history, characterIntent }) {
  ...
  if (['emotional_support','conflict_repair','romance','playful_flirt'].includes(scene) && characterIntent?.strength >= 65) {
...
export function directConversation({ userText = '', brain = null, dialogueState = null, history = [], explicitReplyTarget = null, characterIntent = null, relationshipIntent = null } = {}) {
  const silence = silenceEligibility({ userText, brain, dialogueState, explicitReplyTarget, history, characterIntent });
  const scene = dialogueState?.scene || brain?.activeScene?.type || 'everyday';
  ...
  const forceAgency = !silence.allowed && ((dialogueState?.reactiveStreak || 0) >= 2 || characterIntent?.move === 'take_control');
  return {
    version: 'rin-conversation-director-v2-emotional-inertia',
    scene,
    phase,
