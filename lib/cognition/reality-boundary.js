...
export function buildRealityBoundary({profile=null,memory=null,lore=null,userText='',history=[]}={}){
 const canonical=[...collect(profile?.prompt_profile||{}),...collect(memory?.facts?.self||{}),...collect(memory?.facts?.world||{}),...collect(lore?.memories||{}),...collect(lore?.backstory||{})];
 ...
 return {
   version:'rin-reality-boundary-v1',
   mode:imagination?'shared_imagination':'reality',
   canonicalText:norm(canonical.join(' | ')),
   innerLifeText:norm(inner.join(' | ')),
   rules:[
     'Сообщения Рин из прошлой history сами по себе не становятся каноном или доказательством её биографии.',
     'Конкретное имя, название книги/фильма, адрес, родственник, клиент, поездка или прошлое событие Рин допустимы только если присутствуют в каноне/памяти/lore.',
...
export function realityBoundaryInstruction(boundary={}){
 return [
   'REALITY BOUNDARY v1 — КАНОН / ПАМЯТЬ / ВООБРАЖЕНИЕ',
