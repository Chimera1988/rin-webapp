...
const TRAILING_DEPENDENCY = /(?:^|\s)(?:и|а|но|или|что|чтобы|если|когда|потому|поскольку|хотя|ведь|для|без|с|со|к|по|из|от|до|у|о|об|про|при|над|под|между|через|кроме)(?:\s*[.…!?]*)$/iu;

const REWRITEABLE_REALIZATION_WARNINGS = new Set([
  'user_feminine_address',
  'duplicate_text_segments',
  'recent_assistant_duplicate',
  'recent_assistant_near_duplicate',
  'unplanned_question',
  'missing_required_question',
  'missing_natural_question',
  'too_many_questions'
]);
...
function unfinishedSegment(value = '') {
  const text = String(value || '').trim();
  if (!text) return false;
  if (/[,:;—–-]\s*$/u.test(text)) return true;
  if (TRAILING_DEPENDENCY.test(text)) return true;
  const opens = (text.match(/[«(\[]/gu) || []).length;
  const closes = (text.match(/[»)\]]/gu) || []).length;
  return opens > closes;
}

export function classifyRealizationWarnings(warnings = []) {
  const unique = [...new Set((Array.isArray(warnings) ? warnings : []).map(String).filter(Boolean))];
  const rewriteableWarnings = [];
  const hardWarnings = [];
  for (const warning of unique) {
    const rewriteable = REWRITEABLE_REALIZATION_WARNINGS.has(warning)
      || /^segment_\d+_(?:too_long|unfinished)$/u.test(warning);
    (rewriteable ? rewriteableWarnings : hardWarnings).push(warning);
  }
  return { hardWarnings, rewriteableWarnings };
}
...
export function validateRealization(realization = null, { decision = null, realityBoundary = null, recentHistory = [], currentUserText = '' } = {}) {
  ...
  for (let index = 0; index < actual.length; index += 1) {
    const text = String(actual[index]?.text || '').trim();
    const maxChars = Number(expected[index]?.maxChars) || 5000;
    if (text.length > maxChars) warnings.push(`segment_${index}_too_long`);
    if (unfinishedSegment(text)) warnings.push(`segment_${index}_unfinished`);
  }
  ...
  const classified = classifyRealizationWarnings(warnings);
  return {
    version: 'rin-turn-validator-v2',
    passed: warnings.length === 0,
    retryable: warnings.length > 0 && classified.hardWarnings.length === 0,
    warnings: [...new Set(warnings)],
    hardWarnings: classified.hardWarnings,
    rewriteableWarnings: classified.rewriteableWarnings,
    reply: joined
  };
}
