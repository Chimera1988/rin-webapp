...
function rememberedLoops(memory = null) {
  const canonical = Array.isArray(memory?.conversationState?.openLoops) ? memory.conversationState.openLoops : [];
  const durable = Array.isArray(memory?.openLoops) ? memory.openLoops : [];
  return [...canonical.map(item => ({ ...item, _canonical: true })), ...durable]
    .map(item => normalizeOpenLoop({
      ...
      source: item?._canonical ? (item.source || 'conversation_state') : 'long_term_memory'
    }))
