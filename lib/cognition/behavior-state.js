...
const PLAYFUL_MISREAD_RE = /(?:не\s+понимаю|я\s+(?:тебя\s+)?не\s+понял|что\s+ты\s+(?:имеешь\s+в\s+виду|хочешь\s+сказать)|как\s+это\s+(?:понимать|понять)|в\s+смысле|как\s+(?:это|так)\s+(?:сделать|показать|доказать)|ты\s+(?:обиделась|злишься|сердишься)|я\s+что[-\s]?то\s+не\s+то\s+(?:сказал|сделал)|я\s+тебя\s+чем[-\s]?то\s+(?:обидел|задел))/iu;
const PLAYFUL_ACTS = new Set(['playful_tease', 'playful_mock_offense', 'playful_mischief', 'flirt_challenge', 'flirt_softly']);
...
function noveltySnapshot(recentActs = []) {
  ...
  return {
    recentActs: acts.slice(-6),
    repeatedAct: lastAct,
    streak,
    appearances,
    pressure,
    guidance: pressure >= 70
      ? 'Сцена может продолжаться, но лучше реализовать тот же мотив другим конкретным социальным ходом, если это естественно.'
      : pressure >= 40
        ? 'Не ломать удачную сцену; при возможности слегка разнообразить способ выражения того же намерения.'
        : 'Поведенческая вариативность не требует вмешательства.'
  };
}

function socialMisreadSnapshot(userText = '', recentActs = [], brain = null) {
  ...
  const explicitConfusion = PLAYFUL_MISREAD_RE.test(text);
  const relationalWorry = /(?:ты\s+(?:обиделась|злишься|сердишься)|я\s+что[-\s]?то\s+не\s+то\s+(?:сказал|сделал)|я\s+тебя\s+чем[-\s]?то\s+(?:обидел|задел))/iu.test(text);
  const hidden = clean(brain?.hiddenIntent?.type, 100).toLowerCase();
  const brainConcern = ['possible_hurt_or_withdrawal', 'masked_disappointment'].includes(hidden);
  let risk = 0;
  if (explicitConfusion) risk = playfulContext ? 88 : 34;
  if (relationalWorry) risk = Math.max(risk, playfulContext ? 100 : 76);
  if (brainConcern && playfulContext) risk = Math.max(risk, 82);
  const level = risk >= 80 ? 'high' : risk >= 55 ? 'medium' : risk >= 25 ? 'low' : 'none';
  return {
    risk,
    level,
    playfulContext,
    ...
    explicitConfusion,
    guidance: risk >= 80
      ? 'Пользователь мог буквально прочитать игровую реплику. Не прекращай флирт, но на этом ходу дай один ясный тёплый маркер игры вместо нового абстрактного испытания.'
      : risk >= 55
        ? 'Сохрани игру, но сделай намерение чуть более читаемым через конкретный флирт, самоиронию или мягкий жест.'
        : 'Дополнительное прояснение игрового тона не требуется.'
  };
}
...
export function buildBehaviorState({ userText = '', history = [], brain = null, recentActs = [] } = {}) {
  const text = clean(userText, 1600);
  ...
  const socialMisread = socialMisreadSnapshot(text, recentActs, brain);
  ...
  return {
    ...
    novelty: noveltySnapshot(recentActs),
    socialMisread,
