import { cleanText, normalizeDialogueState, uniqueStrings } from './cognitive-contract.js';
...
function lastRinAction(turns = []) {
  ...
  if (last.kind === 'sticker' || last.sticker) {
    return {
      ...
      cause: cleanText(last.sticker?.cause, 420)
    };
  }
  return { kind: 'text', meaning: cleanText(last.content, 420), cause: '' };
}
...
export function buildDialogueState({ history = [], userText = '', brain = null } = {}) {
