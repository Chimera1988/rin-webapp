...
export function buildDialogueState({ history = [], userText = '', brain = null, explicitReply = null, previousState = null } = {}) {
  ...
  return normalizeDialogueState({
    ...
    }) : (!hasPriorAssistantContext ? prior?.sceneAnchor || null : null),
    openHook: brain?.activeScene?.openHook ? normalizeMessageTarget({
      ...
      confidence: brain.activeScene.continuityStrength || 0.8
    }) : (!hasPriorAssistantContext ? prior?.openHook || null : null),
    ...
    entities: uniqueStrings([...(prior?.entities || []), ...currentEntities], 12, 180),
...
export function dialogueStateInstruction(state = {}) {
  const lines = [
    ...
    ['sticker', 'silence'].includes(state.lastRinAction?.kind)
