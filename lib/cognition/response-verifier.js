...
const SEVERE = new Set([
  ...
  'persistent_intent_abandoned',
  'persistent_intent_deferred',
  'persistent_intent_next_move_not_fulfilled',
  'persistent_intent_scene_binding_missed',
  'unsupported_rin_autobiographical_claim',
  'unsupported_named_self_detail'
]);
...
const BINDING_EVIDENCE = {
  shared_kitsune_identity: /(?:кицун|лис|хвост|облик|япон|миф|легенд|хитрост|защит)/iu,
  shared_imagined_world: /(?:мир|мест|река|горы|лес|тропа|дом|ноч|свет|вместе|мы|нас|рядом)/iu,
  personal_secret_reveal: /(?:\bя\b|\bмне\b|мой|моя|иногда я|люблю|представляю|фантазир|мечтаю)/iu,
  quiet_evening_preferences: /(?:\bя\b|\bмне\b|чай|музык|тишин|вечер|слуш|читать|гулять|окно|свет)/iu,
  affection_physical_closeness: /(?:обним|целу|приж|рядом|улыб|нежн|держ)/iu,
  playful_tease: /(?:поймала|не выкручив|попробуй|посмотрим|смущ|дразн|хитр|не всё сразу|очередь|условие)/iu
};
function bindingMissed(reply, intent) {
  const key = intent?.sceneBinding?.key;
  if (!key || !BINDING_EVIDENCE[key]) return false;
  return !BINDING_EVIDENCE[key].test(reply);
}
const GENERIC_TOPIC_HANDOFF = /(?:о ч[её]м (?:ты )?хочешь поговорить|что хочешь обсудить|давай просто поболтаем|о ч[её]м поговорим|выбирай тему|чем займ[её]мся|что будем делать)/iu;
const INTENT_DEFERRAL = /(?:потом (?:покажу|расскажу|продолжим)|когда-нибудь|ещ[её] успеем|в следующий раз|как-нибудь потом)/iu;
function persistentIntentWarnings(reply, plan) {
  const intent = plan?.rinIntent;
  if (!intent || intent.status !== 'active' || Number(intent.commitment) < 55) return [];
  const warnings = [];
  if (GENERIC_TOPIC_HANDOFF.test(reply)) warnings.push('persistent_intent_abandoned');
  if (plan?.responseAct === 'advance_persistent_intent' && INTENT_DEFERRAL.test(reply)) warnings.push('persistent_intent_deferred');
  if (plan?.responseAct === 'advance_persistent_intent' && bindingMissed(reply, intent)) warnings.push('persistent_intent_scene_binding_missed');
  if (intent.nextMove === 'reveal_promised_special_thing') {
    const reveal = /(?:(?:^|\s)я\s+(?:хотела|хочу|имела|собиралась)|(?:^|\s)мне\s+(?:нравится|хочется|важно|дорого)|(?:^|\s)это\s+(?:то,?\s+)?что\s+я)/iu.test(reply);
    if (!reveal || /(?:может быть|давай уточним|а что ты думаешь|как ты думаешь|попробуй ещё|ещ[её] попыт)/iu.test(reply) || /\?/.test(reply)) warnings.push('persistent_intent_next_move_not_fulfilled');
  }
  return warnings;
}
function realityWarnings(reply, realityBoundary) {
  const violation = unsupportedAutobiographicalClaim(reply, realityBoundary || {});
  return violation ? [violation.type] : [];
}
...
function rewriteGuidance(warnings = [], plan = {}) {
  ...
  if (warnings.includes('persistent_intent_abandoned')) guidance.push('Не отдавай выбор темы пользователю: активная цель Рин ещё не завершена. Продвинь её конкретным следующим ходом.');
  if (warnings.includes('persistent_intent_deferred')) guidance.push('Не откладывай активную цель на потом. Выполни следующий шаг намерения прямо в текущей реплике.');
  if (warnings.includes('persistent_intent_scene_binding_missed')) guidance.push('Не заменяй конкретный target общей теплотой/игривостью. Упомяни или действием продвинь именно sceneBinding активного намерения.');
  if (warnings.includes('persistent_intent_next_move_not_fulfilled')) guidance.push('Выполни именно nextMove активного намерения. Если Рин обещала раскрыть что-то после догадки пользователя — раскрой это сейчас прямой личной фразой, без нового вопроса, отсрочки или ещё одного раунда угадывания.');
  if (warnings.includes('unsupported_rin_autobiographical_claim') || warnings.includes('unsupported_named_self_detail')) guidance.push('Удали выдуманную биографическую конкретику Рин. История ассистента не является доказательством канона. Если источник отсутствует, сохрани только реально заданную activity/мысль либо явно переведи фрагмент в условное воображение («я бы», «представляю»), не придумывая новую книгу, место, родственника или прошлое событие.');
  if (!guidance.length) guidance.push(`Выполни речевой акт ${plan?.responseAct || 'direct_response'} коротко и конкретно.`);
  return guidance;
}

export function verifyReply(rawReply = '', { plan = null, brain = null, userText = '', realityBoundary = null } = {}) {
  ...
  warnings.push(...persistentIntentWarnings(reply, plan));
  warnings.push(...realityWarnings(reply, realityBoundary));
  ...
  return {
    version: 'rin-response-verifier-v11-reality-boundary',
