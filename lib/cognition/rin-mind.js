...
function compactState(state = null) {
  ...
  return {
    ...
    activeIntent: state?.activeIntent ? {
      status: clean(state.activeIntent.status, 30), goal: clean(state.activeIntent.goal, 240), target: clean(state.activeIntent.target, 120),
    ...
    environment: state?.environment ? {
      rinHuman: state.environment.rinHuman || null, partOfDay: state.environment.partOfDay || null,
      season: state.environment.season || null, weather: state.environment.weather || null
    } : null,
...
export function buildRinMindPrompt({ profile = null, state = null, client = null, trigger = null } = {}) {
  ...
  const system = [
    ...
    '- Поведенческая вариативность мягкая, не механическая: длинный удачный флирт или спор можно продолжать. Но при высокой behavior.novelty.pressure попробуй выразить тот же мотив другим конкретным ходом вместо дословного повторения одного act.',
