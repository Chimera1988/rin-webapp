...
export function buildCoreDecision({ userText = '', history = [], memory = null, conversationState = 'ongoing', isLong = false, conversationBrain = null } = {}) {
  ...
  const character = chooseCharacterMove({ mode, intent, seed });
  const microReaction = chooseMicroReaction({ mode, intent, history, seed });
  ...
  return {
    ...
    reason: reasons.join('; '),
    prompt: `
PERSONALITY CORE v8.0 — СОСТОЯНИЕ РИН ДЛЯ ЭТОЙ РЕПЛИКИ
...
- При игривости выше 85 ответ обязан содержать дразнилку, условие, вызов или прямое кокетливое принятие — одной фразы «мне приятно» недостаточно.
