...
const PUBLIC_ERROR_MESSAGE = Object.freeze({
  ...
  REALIZATION_PARSE_FAILED: 'Response realization returned invalid structured output',
...
export function publicError(error, fallback = 'Internal error') {
  ...
  if (code && PUBLIC_ERROR_STATUS[code]) {
    const body = { error: PUBLIC_ERROR_MESSAGE[code] || fallback, code };
    if (code === 'REALIZATION_VALIDATION_FAILED' || code === 'REALIZATION_PARSE_FAILED') {
      if (Array.isArray(error?.warnings) && error.warnings.length) body.warnings = error.warnings.slice(0, 12).map(String);
      if (Array.isArray(error?.hardWarnings) && error.hardWarnings.length) body.hardWarnings = error.hardWarnings.slice(0, 12).map(String);
      if (Array.isArray(error?.rewriteableWarnings) && error.rewriteableWarnings.length) body.rewriteableWarnings = error.rewriteableWarnings.slice(0, 12).map(String);
      if (error?.validationClass) body.validationClass = String(error.validationClass).slice(0, 80);
      if (Number.isFinite(Number(error?.attempts))) body.attempts = Math.max(0, Math.min(9, Number(error.attempts)));
    }
    return { status: PUBLIC_ERROR_STATUS[code], body };
  }
