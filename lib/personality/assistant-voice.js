...
const AGENCY_DEFERRAL = /(?:готовься(?:\s+к)?|приготовься|держись(?:\s+крепче)?|пристегни\s+ремни|мы\s+(?:начинаем|начн[её]м)|(?:^|[.!…]\s*)начинаем(?:[.!…]|$)|будет\s+(?:весело|интересно|увлекательно)|это\s+только\s+начало|к\s+неожиданностям|я\s+(?:найду|придумаю|выберу)\s+способ)/iu;
const CONCRETE_AGENCY = /(?:перв(?:ый|ое)\s+(?:ход|правило)|сам\s+попросил|поздно\s+передумывать|инициативу\s+я\s+забрала|не\s+выкручивайся|(?:^|[.!…]\s*)(?:попробуй|скажи|смотри|подойди|закрой|выбери|останься|стой|сиди|не\s+торопись|не\s+отвлекайся|не\s+прячься|не\s+улыбайся)(?=\s|[,.!…]|$))/iu;
...
export const ASSISTANT_VOICE_PATTERNS = Object.freeze({
  ...
  symmetricEcho: /^(?:тогда\s+)?(?:вот и я|я тоже|взаимно|а можно тогда|тогда вот|и мне тоже|тоже самое)/iu,
  overExplanation: /(?:это (?:означает|помогает|добавляет|делает)|потому что|таким образом|в конечном итоге)/iu,
  emptyEnthusiasm: /^(?:здорово|отлично|замечательно|прекрасно|супер|точно)[!., ]/iu
});
...
export function analyzeAssistantVoice(value = '', { userText = '', scene = '', plan = null } = {}) {
  ...
  const flags = {
    ...
    symmetricEcho: ASSISTANT_VOICE_PATTERNS.symmetricEcho.test(text),
    overExplanation: ASSISTANT_VOICE_PATTERNS.overExplanation.test(text) && text.length > 120,
    emptyEnthusiasm: ASSISTANT_VOICE_PATTERNS.emptyEnthusiasm.test(text),
    endsWithQuestion: /\?\s*$/u.test(text),
    longQuestion: /\?\s*$/u.test(text) && text.length > 78,
    mirrored: false,
    agencyDeferral: false,
    concreteAgency: false
  };
  ...
  flags.concreteAgency = agencyAct && CONCRETE_AGENCY.test(text);
  flags.agencyDeferral = agencyAct && AGENCY_DEFERRAL.test(text) && !flags.concreteAgency;
  ...
  const score = [
    ...
    flags.symmetricEcho ? 3 : 0,
    flags.overExplanation ? 2 : 0,
    flags.emptyEnthusiasm ? 2 : 0,
    flags.mirrored ? 4 : 0,
    flags.longQuestion ? 1 : 0,
    flags.agencyDeferral ? 4 : 0
  ...
  const lacksAgency = agencyAct && (flags.agencyDeferral || (!ownAgencyMarker && !flags.concreteAgency && !/^[^?]{1,80}[.!…]?$/u.test(text)));
  return { version: 'rin-voice-guard-v4-agency-follow-through', text, flags, score, overlap, reactive: reactive || lacksAgency, personalAct, agencyAct, ownAgencyMarker, lacksAgency };
}
