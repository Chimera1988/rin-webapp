...
export function selectRelevantMemory(memory, userText = '', history = [], limits = {}) {
  if (!memory || typeof memory !== 'object') return { facts: [], events: [] };
  const context = [userText, ...history.slice(-6).map(x => x?.content || '')].join(' ');
  ...
  return { facts: selectedFacts, events };
}
...
export function continuityInstruction(snapshot) {
  ...
  const lines = [
    ...
    snapshot.unresolved?.length ? `Незавершённые линии Кирилла:\n- ${snapshot.unresolved.join('\n- ')}` : '',
