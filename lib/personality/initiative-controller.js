...
export function chooseInitiative({ userText = '', history = [], intent = 'connection', conversationBrain = null, rhythm = null, seed = '' } = {}) {
  ...
  const turnCount = history.filter(x => x?.role === 'assistant').length;
  ...
  if (turnCount >= 6 && roll >= 18 && roll < 28) {
    return {
      ...
      instruction: 'После ответа допустимо добавить одно маленькое бытовое наблюдение Рин, возникшее сейчас. Оно должно быть конкретным, коротким и связанным с текущей темой.'
    };
  }
