...
const OPENINGS = {
  ...
  gentle: ['', '', 'Знаешь...', 'Почему-то...', 'Мм...', 'Ну вот...', 'Это приятно.'],
...
export function chooseHabits({ mode, intent, history = [], seed = '' } = {}) {
  ...
  const opening = allowOpening ? chooseFresh(OPENINGS[mode] || OPENINGS.calm, history, `${seed}:opening`) : '';
  const rhythm = pickStable(RHYTHMS[mode] || RHYTHMS.calm, `${seed}:rhythm`);
  const fragmentChance = ['playful', 'bold_playful', 'shy_tender', 'thoughtful'].includes(mode) && turn % 4 === 1;
