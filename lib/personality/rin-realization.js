...
export function buildRealizationRetryInstruction(warnings = [], decision = null, previousRealization = null, rewriteAttempt = 1) {
  ...
  const previous = previousRealization && typeof previousRealization === 'object'
    ? clean(JSON.stringify(previousRealization), 6500)
    : '';
  const rules = [
    `Переформулировка ${Math.max(1, Number(rewriteAttempt) || 1)}/2. Предыдущая формулировка нарушила deterministic validation.`,
    `Нарушения: ${list.join(', ') || 'unknown'}.`,
    previous ? `Предыдущий отклонённый текст (НЕ повторяй его дословно):\n${previous}` : '',
    'Реализуй ТО ЖЕ TurnDecision заново; act, intent, delivery, question.mode, количество сегментов, purpose и факты не меняй.',
    ...
    list.some(item => /unfinished/.test(item)) ? 'Нарушивший сегмент синтаксически не завершён. Закончи его как самостоятельное сообщение; не оставляй в конце союз, предлог, тире, открытую скобку/кавычку или зависимую конструкцию.' : '',
    ...
    list.includes('missing_natural_question') ? `TurnDecision выбрал question.mode=natural${decision?.question?.reason ? ` (${clean(decision.question.reason, 220)})` : ''}. Добавь ровно один естественный вопрос, вытекающий из purpose и текущего контекста; не превращай его в анкету и не меняй сам TurnDecision.` : '',
    list.includes('missing_required_question') ? 'TurnDecision выбрал question.mode=required. Добавь ровно один необходимый уточняющий вопрос, не меняя остальной смысл решения.' : '',
    list.includes('unplanned_question') ? 'TurnDecision выбрал question.mode=none. Удали вопросительную конструкцию; сохрани утверждение/реакцию без вопроса.' : '',
    list.includes('too_many_questions') ? 'Оставь только ОДИН самый содержательный вопрос, предусмотренный TurnDecision; остальные преобразуй в утверждения или убери.' : '',
    ...
    'Исправляй ТОЛЬКО перечисленные нарушения. Не добавляй новые вопросы, факты, обещания или отдельные conversational beats.'
  ].filter(Boolean);
  return rules.join('\n');
}
