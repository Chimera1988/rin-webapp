...
export function innerLifeInstruction(snapshot = {}) {
  ...
  const lines = [
    ...
    snapshot.directQuestion
      ? 'пользователь прямо спрашивает о её дне или занятии: ответь конкретно от первого лица, используя текущую деятельность. Допустима одна незавершённая или бытовая деталь. Не отвечай общими словами «всё спокойно».'
      : snapshot.spontaneousAllowed
        ...
        : 'Не упоминай текущую деятельность без причины: она уже звучала недавно либо сейчас важнее реплика пользователя.',
