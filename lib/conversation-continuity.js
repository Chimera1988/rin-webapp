...
function usableTurns(history = [], max = 48) {
  return (Array.isArray(history) ? history : [])
    ...
    .map((item, index) => ({
      ...
      role: item.role,
      kind: item.kind || 'text',
      content: item.kind === 'sticker'
        ? clean(item.sticker?.meaning || item.sticker?.emotion || item.content, 600)
        : clean(item.content, 1800),
...
function recentAssistantDiagnostics(turns = []) {
  const recent = turns.filter(item => item.role === 'assistant' && item.kind !== 'sticker').slice(-5);
