...
/* Conversation Perception v3.
 * Deterministic observation layer before the Cognitive Kernel.
 * It may classify meaning, relation, ambiguity and scene continuity, but it never
 * prescribes what Rin should do next.
 */
...
export function analyzeConversation({ userText = '', history = [], conversationState = 'ongoing' } = {}) {
  ...
  return {
    version: 'conversation-perception-v3',
    literalIntent,
    hiddenIntent,
    relation,
    activeScene: {
    ...
    referents,
    ambiguity: { level: clamp(ambiguity) },
    summary: [
