import { isExplicitFarewell, isTextTurn } from './chat-contract.js';
...
function recentTurns(history = [], count = 10) {
  return (Array.isArray(history) ? history : [])
    .filter(item => isTextTurn(item) || item?.kind === 'sticker')
    .slice(-count)
    .map(item => ({ role: item.role, kind: item.kind || 'text', content: textOf(item.content).slice(0, 1800) }));
}
...
function detectHiddenIntent(userText, turns, literalIntent) {
  ...
  if (previousSticker && /^(ты чего|что случилось|в чём дело|почему|обиделась|ревнуешь|ты злишься|смутилась)\??$/iu.test(t)) {
    return set('ask_about_previous_nonverbal', 96, ['пользователь спрашивает о последнем невербальном жесте Рин']);
  }
...
function detectReferents(userText) {
  ...
  if (/\b(я|меня|мне|мой|моя|мои)\b/i.test(t)) refs.push('user');
...
export function analyzeConversation({ userText = '', history = [], conversationState = 'ongoing' } = {}) {
  ...
  return {
    version: 'conversation-brain-v1.1',
    literalIntent,
    hiddenIntent,
    relation,
    activeScene: {
...
export function conversationBrainInstruction(brain) {
  ...
  return `
CONVERSATION BRAIN v1.1 — СМЫСЛ ТЕКУЩЕГО ХОДА
Сцена: ${brain.activeScene.type}; тема: ${brain.activeScene.topic}; явное намерение: ${brain.literalIntent}; подтекст: ${brain.hiddenIntent.type} (${brain.hiddenIntent.confidence}%); связь: ${brain.relation.type}; направление: ${brain.activeScene.emotionalDirection}.
Фокус: ${brain.responseFocus}

ОБЯЗАТЕЛЬСТВА:
${obligations}

Сначала выполни эти смысловые обязательства, затем применяй Personality Core. Учитывай предыдущую реплику при зависимом ответе; не придумывай конфликт при низкой уверенности и не раскрывай анализ пользователю.
`.trim();
}
