...
function isExplicitFarewell(userText) {
  const t = lower(userText);
  if (/(?:до встречи|до завтра|спокойной ночи|доброй ночи|увидимся|до связи|бай|bye)/i.test(t)) return true;
  // «Пока» считается прощанием только как самостоятельная завершающая реплика,
  // а не в выражениях «пока работаю», «пока что», «пока не спал».
  return /^(?:(?:ну|ладно)[, ]+)?(?:(?:всё|все)[, ]+)?пока[.!…)]*$/i.test(t);
}
