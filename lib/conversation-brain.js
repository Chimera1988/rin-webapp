...
function isExplicitFarewell(userText) {
  ...
  return /(?:^|\s)(?:пока|до встречи|до завтра|спокойной ночи|доброй ночи|увидимся|бай|bye)(?:[.!…\s]|$)/i.test(t);
}
...
function detectScene(turns, userText) {
  ...
  if (isExplicitFarewell(t)) add('farewell', 95);
...
export function conversationBrainInstruction(brain) {
  ...
  const obligations = brain.obligations?.length
    ...
    : '- Ответь на явный смысл и сохрани непрерывность разговора.';
