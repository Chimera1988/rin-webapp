...
function detectHiddenIntent(userText, turns, literalIntent) {
  ...
  if (/(поприста|пристава|можешь начинать|начинай|весь в нетерпении|твоя очередь|давай пофлирт|пофлиртуем)/i.test(t)) {
    return set('invite_rin_initiative', 94, ['пользователь прямо передаёт Рин инициативу внутри игривой сцены']);
  }
...
function detectRelation(userText, turns, literalIntent) {
  ...
  const startsLikeNewQuestion = /^(?:кто|где|когда|сколько|какой|какая|какие|чей|чья|чьи|у тебя|ты)/iu.test(t);
