...
test('legacy top-level open loops migrate once into canonical ConversationState and are resolved only by state transition id', async () => {
  storage.setItem('rin-diary-v1', JSON.stringify({
    _schema: 4,
    openLoops: [
      { id:'loop-a', text:'Заказать билет в Берлин', type:'plan', importance:70 },
      { id:'loop-b', text:'Билет', type:'topic', importance:40 }
    ]
  }));
  ...
  assert.deepEqual(diary.conversationState.openLoops.map(item => item.id), ['loop-b']);
});
...
test('prepareInnerLife is pure until a successful turn is committed', async () => {
  ...
  const committed = await memory.commitTurnState({
    ...
    now: 1_000_000,
    stateTransition: {
      moodState: { affection: before.mood.affection + 2, energy: before.mood.energy - 1 },
      relationshipState: { ...before.relationship, trust: before.relationship.trust + 1, closeness: before.relationship.closeness + 2 },
...
test('commitTurnState is idempotent for the same request and state survives reload', async () => {
  ...
  const first = await memory.commitTurnState({
    ...
    stateTransition: {
      ...
      moodState: { affection: initial.mood.affection + 5, energy: initial.mood.energy },
      relationshipState: { ...initial.relationship, trust: initial.relationship.trust + 4 }
    }
  });
  ...
  const duplicate = await memory.commitTurnState({
    ...
    now: 2_000_100,
    stateTransition: {
      moodState: { affection: 100, energy: 0 },
      relationshipState: { ...initial.relationship, trust: 100 }
    }
  });
...
test('persistent Rin intent survives transactional commit and reload exactly', async () => {
  const rinIntent = {
    schema: 'rin-persistent-intent-v4', id: 'intent-persist', status: 'active',
    ...
    startedAtTurn: 2, updatedAtTurn: 3, turnCount: 1, minTurns: 2, maxTurns: 4, source: 'cognitive_kernel'
  };
...
test('duplicate request cannot advance persistent intent twice', async () => {
  const intent = { schema:'rin-persistent-intent-v4', id:'intent-idempotent', status:'active', goal:'продвинуть игровую линию', motive:'play', target:'shared_playful_scene', scene:'playful_flirt', priority:80, commitment:80, progress:.3, nextMove:'tease_or_advance', completionCondition:'done', abandonmentCondition:'stop', startedAtTurn:1, updatedAtTurn:1, turnCount:1, minTurns:2, maxTurns:4, source:'cognitive_kernel' };
